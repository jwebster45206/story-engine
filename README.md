...
`/v1` routes require `Authorization: Bearer <key>`. Keys are UUIDs you generate (`uuidgen`) and list in config as `api_keys` (own only the games they create) and `admin_keys` (any gamestate).
